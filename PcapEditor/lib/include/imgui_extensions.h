...
namespace ImGui {
    ...
    void TextFormatted(const std::string &fmt, auto &&...args) {
        ImGui::TextUnformatted(utility::format(fmt, std::forward<decltype(args)>(args)...).c_str());
    }

    void TextFormattedColored(ImColor color, const std::string &fmt, auto &&...args) {
        ImGui::TextColored(color, "%s", utility::format(fmt, std::forward<decltype(args)>(args)...).c_str());
    }

    void TextFormattedDisabled(const std::string &fmt, auto &&...args) {
        ImGui::TextDisabled("%s", utility::format(fmt, std::forward<decltype(args)>(args)...).c_str());
    }

    void TextFormattedWrapped(const std::string &fmt, auto &&...args) {
        ImGui::TextWrapped("%s", utility::format(fmt, std::forward<decltype(args)>(args)...).c_str());
    }

    void TextFormattedCentered(const std::string &fmt, auto &&...args) {
        auto text = utility::format(fmt);
